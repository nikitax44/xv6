...
// Free a disk block.
static void bfree(int dev, u32 b) {
  struct buf* bp;
  int         bi, m;
...
int namecmp(str s, str t) { return strncmp(s, t, DIRSIZ); }
...
//
static str skipelem(str path, char* name) {
  str s;
  int len;
...
static struct inode* namex(str path, int nameiparent, char* name) {
