...
void copyinstr2(char* s) {
  ...
  ret          = exec(b, args);
  if (ret != E2BIG) {
    printf("exec(%s) returned %d, not E2BIG\n", b, fd);
    exit(1);
  }
  ...
  if (pid == 0) {
    ...
    ret           = exec("echo", args2);
    if (ret != E2BIG) {
      printf("exec(echo, BIG) returned %d, not E2BIG\n", fd);
      exit(1);
    }
    exit(747); // OK
  }
...
void truncate1(char* s) {
  ...
  int n   = read(fd2, current_buf, sizeof(current_buf));
  if (n != 4) {
  ...
  n       = read(fd3, current_buf, sizeof(current_buf));
  if (n != 0) {
    printf("aaa fd3=%d\n", fd3);
  ...
  n = read(fd2, current_buf, sizeof(current_buf));
  if (n != 0) {
    printf("bbb fd2=%d\n", fd2);
    printf("%s: read %d bytes, wanted 0\n", s, n);
    exit(1);
...
void truncate3(char* s) {
  ...
  if (pid == 0) {
    for (int i = 0; i < 100; i++) {
      ...
      read(fd, current_buf, sizeof(current_buf));
      close(fd);
    }
    exit(0);
  }
...
void sharedfd(char* s) {
  ...
  for (i = 0; i < N; i++) {
    if (write(fd, current_buf, sizeof(current_buf)) != sizeof(current_buf)) {
  ...
  nc = np = 0;
  while ((n = read(fd, current_buf, sizeof(current_buf))) > 0) {
...
void sbrkbugs(char* s) {
  ...
  if (pid == 0) {
    ...
    sbrk((int)((10 * 4096 + 2048) - (u64)sbrk(0)));
...
void diskfull(char* s) {
  ...
  for (fi = 0; done == 0 && '0' + fi < 0177; fi++) {
    ...
    for (int i = 0; i < MAXFILE; i++) {
      ...
      if (write(fd, current_buf, BSIZE) != BSIZE) {
        done = 1;
        close(fd);
        break;
      }
